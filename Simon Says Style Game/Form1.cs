...
namespace Simon_Says_Style_Game
{
        ...
        private void GameTimerEvent(object sender, EventArgs e)
        {
            ...
            if (tries >= level)
            {
                ...
                if (correctOrder == playerOrder)
                {
                    tries = 0;
                    GameTimer.Stop();
                    MessageBox.Show("Well done you got them correctly");
                    score++;
                }
                else
                {
                    tries = 0;
                    GameTimer.Stop();
                    MessageBox.Show("Your Guesses did not match, Try Again");
                }
            }

            label2.Text = "Click on " + level + " Blocks in the same sequence";
